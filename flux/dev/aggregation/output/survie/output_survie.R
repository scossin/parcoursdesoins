...
observeEvent(input$gosurvie,{
  ...
  for (i in 1:length(values)){
    ...
    for (y in 1:length(filtres)){
      ...
      valeurs_id <- filtres[[y]]$get_df_selection()[,colonne]
      ...
      ajout <- subset (filtres[[y]]$get_df_selection(), select=colonnes)
    ...
    date_auj <- as.POSIXct(Sys.Date())
    df_survie$datestartevent2 <- ifelse (bool, df_survie$datestartevent2,
                                         date_auj) ## date du jour ??? : quel serait la date de dernier contact : à quel rythme remonte les données sur les décès ??
    df_survie$datestartevent2 <- as.POSIXct(df_survie$datestartevent2, origin = "1970-01-01") ## car NA présents
    
    df_survie$diff_time <- as.numeric(difftime (df_survie$datestartevent2, df_survie$datestartevent))
    ...
    if (is.null(groupe) || groupe == "aucun"){ # premier cas de figure : aucun groupe sélectionné :
      cat(input[["survie_radiobutton_0"]])
...
#      tab <- read.table(textConnection(capture.output(my.fit)),skip=2,header=TRUE)
      ...
      if (bool){
        ...
        for (y in 1:length(filtres)){
          ajout <- subset (filtres[[y]]$df, select=colonnes)
        }
      }
