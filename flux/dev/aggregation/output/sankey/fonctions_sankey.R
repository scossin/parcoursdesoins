...
create_sankey_radiobutton <- function(colonnes_tableau, event_number){
  id <- paste0("sankey_radiobutton_",event_number)
  label <- paste0("event",event_number)
  ...
  liste <- list(HTML("<div><h4> event ",event_number,"</h4>"), ui , HTML("</div>"))
  newlist <- list()
