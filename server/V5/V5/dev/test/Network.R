...
# minimal example
nodes <- data.frame(id = 1:3, label=c("node1","node2","node3"), shape=c("square",
                                                                        "triangle","text"))
edges <- data.frame(from = c(1,2), to = c(1,3), label=c("1to1","2to3"))

visNetwork(nodes, edges)
list.files()
load("../../.rdata")
source("../../classes/queries/QueryVizOO.R")
queryViz <- QueryViz$new(xmlSearchQuery)
