XMLquery <- R6Class(
  ...
  public = list(
    ...
    initialize = function(){
      self$docType <- XML::Doctype(name = "eventslinks", system = "eventslinks.dtd")
    },
    ...
    addPredicateNode = function(eventNumber,predicateClass, predicateType, values, minValue, maxValue){
      ...
      # oldList <- self$listPredicatesNodes[[eventName]]
      # if (is.null(oldList)){
      #   self$listPredicatesNodes[[eventName]] <- list(predicateNode)
      # } else {
      #   self$listPredicatesNodes[[eventName]] <- list(oldList, predicateNode)
      # }
    },
