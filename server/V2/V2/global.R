...
patientsids <- unique(listeevents$patientid)
...
#################################### D3 Tree #############################################
## prend la data.frame titanic : données simulées
dfpatients <- Titanic%>%data.frame
levels(dfpatients$Sex) <- c("Homme","Femme")
levels(dfpatients$Age) <- c("Enfant","Adultes")
dfpatients$Class <- NULL
dfpatients$deces <- dfpatients$Survived
levels(dfpatients$deces) <- c("Non","Oui")
dfpatients$Survived <- NULL
dfpatients$Freq <- NULL
