setRefClass(
  ...
  methods=list(
    ...
    set_df_type_selected = function(df_type_selected){
      ...
      colonne_id <- "patient"
      colonnes_tableau <- c("type","duree","StartToStart","EndToStart","agregat")
    ...
    set_df_events_selected = function(){
      ...
      for (i in length(filtres)){
        df_selection <- rbind (filtres[[i]]$get_df_selection()) ## pb : pas les memes colonnes
      }
    ...
    set_df_nextprevious_events = function(boolnext){
      ...
      if (boolnext){
        df_next_temp$StartToStart <- difftime(df_next_temp$datestartevent,df_next_temp$mainstart,units="day")
        df_next_temp$EndToStart <- difftime(df_next_temp$datestartevent,df_next_temp$mainend,units="day")
      } else {
        df_next_temp$StartToStart <- difftime(df_next_temp$mainstart,df_next_temp$datestartevent,units="day")
        df_next_temp$EndToStart <- difftime(df_next_temp$mainend,df_next_temp$datestartevent,units="day")
      }
      df_next_temp$StartToStart <- as.numeric(df_next_temp$StartToStart)
      df_next_temp$EndToStart <- as.numeric(df_next_temp$EndToStart)
